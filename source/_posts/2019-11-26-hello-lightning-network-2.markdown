...
闪电网络实现了一种基于[Sphinx](http://bit.ly/2q6ZDrP) 方案的洋葱路由协议。该路由协议确保支付发送者可以通过闪电网络构建和通信路径，使得：
