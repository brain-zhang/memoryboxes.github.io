...
也被成为P2SH（Pay-to-Script-Hash）交易。
...
也被称为P2PKH（Pay-to-Public-Key-Hash）交易。

该类是最常见的交易类型，由N个输入、M个输出构成。交易地址都是以`1`开头。这种交易也是目前比特币网络中最典型的交易类型，也最简明，容易分析。下面我们就先拿它作为例子，开始探究一笔比特币交易的细节。
...
![img](https://0dayzh.gitbooks.io/bitcoin_developer_guide/content/en-tx-overview.svg)
...
最后是一个字段是锁定事件(Locktime)。Locktime 允许签名者创建一个时间锁定交易。因为只会在将来生效，这给签名者一个的反悔的机会。
...
![img](https://0dayzh.gitbooks.io/bitcoin_developer_guide/content/en-tx-overview-spending.svg)
...
* vout (输出UTXO)
    ...
    - value: 转账金额
    - n: 作为第N个UTXO输出
    - scriptPubKey: 这是设置的谜题，后来人想要花费这笔UTXO，必须提供scriptSig来解答这个谜题才可以
...
简单说下vin中一笔tx内部结构:
    ...
    - previous_output (32字节): 上一笔交易的HASH值，即这个花费的输入交易ID: 
    ...
    - signature script (整个解密脚本)： 这个结构我们之后会具体分析
```
...
简单说下vout中一笔tx内部结构:
    ...
    - value (8字节): 转账到这个地址上的金额， 这里是`581b000000000000`小端排序，十六进制为`00000000000000581b`，即转账7000 satoshis
    - pk_script length (>=1, var_int类型): 输出脚本的长度，这里是`0x21`，代表33字节长度
    - pk script: 输出脚本，其实就是包含转账地址的脚本，这里是`03db3c3977c5165058bf38c46f72d32f4e872112dbafc13083a948676165cd1603ac`
...
![img](https://ws1.sinaimg.cn/large/893d6e9dly1fxtpsr4wzsj20p60dumzf.jpg)
